...
-- 步骤 5: 插入卡牌游戏模板
-- ============================================
-- 使用 $html$ 作为美元引号标签，避免与 HTML 中的 $ 符号冲突
DO $block$
DECLARE
...
BEGIN
  ...
  IF card_category_id IS NOT NULL THEN
    -- HTML 代码使用 $html$ 标签，避免与 DO 块的 $block$ 冲突
    html_code := $html$
<!DOCTYPE html>
<html>
<head>
...
<body>
  <div class="game-container">
    ...
    <div class="player-area">
      <div class="stats">
        <div class="health">❤️ 生命: <span id="playerHealth">30</span></div>
        <div class="mana">💎 法力: <span id="playerMana">3</span>/10</div>
      </div>
  ...
  <script>
    ...
    let playerHand = [], deck = [];
    const cardTypes = [
      {name: "火球术", cost: 2, attack: 3, type: "spell"},
      {name: "治疗", cost: 1, heal: 5, type: "heal"},
      ...
      playerHand.forEach((card, index) => {
        ...
        cardEl.innerHTML = `
          <div class="card-front">
            ...
            ${card.attack ? `<div class="card-attack">${card.attack}</div>` : ""}
            ${card.health ? `<div class="card-health">${card.health}</div>` : ""}
          </div>
        `;
    ...
    function checkGameOver() {
      if (playerHealth <= 0) {
        document.getElementById("gameOver").innerHTML = "<h2>游戏结束！你输了！</h2><button onclick=\"location.reload()\">再来一局</button>";
      } else if (opponentHealth <= 0) {
        document.getElementById("gameOver").innerHTML = "<h2>恭喜！你赢了！</h2><button onclick=\"location.reload()\">再来一局</button>";
      }
    }
